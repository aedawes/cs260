                        ...
                        <v-card elevation="5" class="card">
                            <v-list-item>
                                <v-list-item-avatar size="200" color="grey"> 
                                    <p id="logo" >{{item.firstName[0]}}{{item.lastName[0]}}</p>
                                </v-list-item-avatar>
                            ...
                            <v-list-item v-show="clicked" dark elecation="5" class="card" id="editCard">
                                ...
                                <v-list-item-content class="inside">
                                    <v-text-field class="input" v-model="form.address" label="Address"/>
                                    <v-text-field class="input" v-model="form.email" label="Email"/>
                                    <v-text-field class="input" v-model="form.phoneNumber" label="Phone Number"/>
                                    <v-text-field class="input" v-model="form.website" label="Website"/>
                                    <p id="spacer"></p>
                                    <v-btn @click="clicked=!clicked, updateForm(item)">Update</v-btn>
                                </v-list-item-content>
                            </v-list-item>
                        </v-card>
                    </div>
...
export default {
  ...
  methods: {
    ...
    async updateForm(card) {
        try {
            this.findCard = card;
            console.log(this.findCard.address)
            console.log(" here: " + this.form.address)
            let email = this.form.email
            let phoneNumber = this.form.phoneNumber
            let website = this.form.website
            let address = this.form.address
            if(this.findCard.email === this.form.email){
                email = this.findCard.email
            }
            if(this.findCard.phoneNumber === this.form.phoneNumber){
                phoneNumber = this.findCard.phoneNumber
            }
            if(this.findCard.website === this.form.website){
                website = this.findCard.website
            }
            if(this.findCard.address === this.form.address){
